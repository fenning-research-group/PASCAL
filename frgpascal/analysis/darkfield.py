...
def load_fids(dir):
    return(frgm.listdir(dir))

def load_image(fid, load_red = True):
    """
...
def get_median(im):
    ...
    if len(im.shape) == 2:
        ...
        im = im[:,:,0]
        print('Careful! Image entered as RGB, not single channel. Red channel was used.')
    else:
        print('The image was not in the correct format. Must be RGB or a single channel')
        pass
