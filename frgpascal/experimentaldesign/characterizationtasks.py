...
class PLImaging(CharacterizationMethod):
    def __init__(self, dwelltimes=[0.05, 0.2, 1, 5], jitter=0):
        for d in dwelltimes:
            if d <= 0 or d > 100:
                raise Exception(
                    f"Invalid dwelltime: {d} seconds. Must be 0<dwelltime<100 seconds"
...
class PLSpectroscopy(CharacterizationMethod):
    def __init__(self, dwelltimes=[0.1, 5, 20], numscans=1, jitter=0):
        for d in dwelltimes:
            if d <= 0 or d > 60:
                raise Exception(
                    f"Invalid dwelltime: {d} seconds. Must be 0<dwelltime<60 seconds"
...
class PLPhotostability(CharacterizationMethod):
    def __init__(self, dwelltime=2, duration=60, jitter=0):
