"""
V1.0 Spincoater code for use with ODrive S1
"""
...
class SpinCoater:
    ...
    def connect(self, **kwargs):
        ...
        if regular_bootup:
            ...
            self.axis.requested_state = (
                AxisState.FULL_CALIBRATION_SEQUENCE  # calibrate the encoder
            )
            ...
            self.axis.requested_state = (
                AxisState.CLOSED_LOOP_CONTROL  # normal control mode
            )
            # odrive defaults
            self.axis.config.motor.current_hard_max = 10  # Amps NOT SAME AS POWER SUPPLY CURRENT. This is targeting ~25% of the specified max motor current
            self.axis.controller.config.circular_setpoints = True  # position = 0-1 radial
            #self.axis.pos_vel_mapper.config.circular_setpoints = True
    ...
    def lock(self):
        ...
        print(f"\tcurrent position: {self.axis.pos_vel_mapper.pos_rel}")
        ...
        time.sleep(self.COMMUNICATION_INTERVAL)
        diff = ((self.__HOMEPOSITION - self.axis.pos_vel_mapper.pos_rel + 0.5) % 1.0) - 0.5
        if np.abs(diff) < 0.05:
            print(np.abs(diff))
            self.axis.controller.input_pos = self.axis.pos_vel_mapper.pos_rel + 0.2
        time.sleep(self.COMMUNICATION_INTERVAL)
        time.sleep(self.COMMUNICATION_INTERVAL)
        diff = ((self.__HOMEPOSITION - self.axis.pos_vel_mapper.pos_rel + 0.5) % 1.0) - 0.5
        self.axis.controller.input_pos = self.axis.pos_vel_mapper.pos_rel + diff
        time.sleep(self.COMMUNICATION_INTERVAL)
        t0 = time.time()
        print(f"\tStarting to find home: \n\t\tinitial time is {t0}")
        while np.abs(((self.__HOMEPOSITION - self.axis.pos_vel_mapper.pos_rel + 0.5) % 1.0) - 0.5) > 0.05:  # tolerance = 360*value degrees, 0.025 ~= 10 degrees
            time.sleep(0.1)
    ...
    def twist_off(self):
        ...
        current_pos = self.axis.pos_vel_mapper.pos_rel
        target_mod = (self.__HOMEPOSITION + self.__TWISTDELTA) % 1.0
        diff = ((target_mod - current_pos + 0.5) % 1.0) - 0.5
        target_position = current_pos + diff
        self.axis.controller.input_pos = target_position
        t0 = time.time()
        while (np.abs(target_position - self.axis.pos_vel_mapper.pos_rel)) > 0.025:
            time.sleep(0.1)
            if time.time() - t0 > self.TIMEOUT:
                print("resetting")
                self.reset()
    ...
    def stop(self):
        ...
        min_stopped_time = 2
        while True:
            if abs(self.axis.pos_vel_mapper.vel) > 0:
                t0 = time.time()
        ...
        while self.__logging_active:
            if self.__connected:
                ...
                self.__logdata["rpm"].append(
                    self.axis.vel_estimate * 60
                )  # rps from odrive -> rpm
                self.__logdata["pos"].append(
                    # self.axis.encoder.pos_estimate
                    self.axis.pos_vel_mapper.pos_rel
    ...
    def __libfibre_timer_worker(self):
        ...
        while self.__connected:
            time.sleep(1)
            libfibre = getattr(self.odrv0, "_libfibre", None)
            if libfibre is not None and not self.__logging_active and len(libfibre.timer_map) > 60:
                try:
