...
void ImageNotificationCallback (int event_id, void *context)
{
   ...
   case TSI_IMAGE_NOTIFICATION_PENDING_IMAGE:
      ...
      if (image)
      {
         // Print a message to the console to indicate that we received the image.
         printf ("Received an image.\n");
         // In this toy example, we simply print the RGB pixel values of a single pixel in the image which executes fast.
         // A realistic application will do something more sophisticated with the image data but *must* also execute fast
         // to stay within the realm of defined behavior.
         // If extensive image processing is required, it is highly recommended that the callback function implementation copy
         // the image from inside this callback for deferred processing elsewhere (e.g. on another thread).
...
int main()
{
   ...
   sdk_handle = ::LoadLibrary ("tsi_sdk.dll");
   if (!sdk_handle)
   {
   ...
   if (num_cameras)
   {
      ...
      if (cam->Open())
      {
         ...
         // Send a software trigger.  Will deliver 5 frames since we set TSI_PARAM_FRAME_COUNT to 5 above.
         cam->SWTrigger();
         Sleep (2000); // Wait long enough to get 5 frames.
